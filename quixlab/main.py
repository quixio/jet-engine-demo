"""Jet Engine Test Rig Dashboard — ported from a Marimo notebook.

Loads thrust-stand telemetry from QuixLake (table ``config-enriched-data``)
and lets you explore campaigns, single test runs, and side-by-side
comparisons of two test runs.

Environment:
    Quix__Sdk__Token   PAT used by ql.sql to talk to QuixLake.
    QUIX_LAKE_URL      (optional) override for the QuixLake API endpoint.
"""
